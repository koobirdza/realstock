...
async function getJson(action, params = {}, { useCache = false, ttlMs = CLIENT_CACHE_TTL_MS } = {}) {
  mustHaveUrl();
  ...
  if (useCache && json?.ok) setCached(key, json, ttlMs);
  return json;
}
export const pingServer = () => getJson("ping");
export const getCatalog = (mode) => getJson("catalog", { mode }, { useCache: true, ttlMs: 10 * 60 * 1000 });
export const getCurrentStockSummary = () => getJson("currentStock", {}, { useCache: true });
export const getOrderView = () => getJson("orderView", {}, { useCache: true });
