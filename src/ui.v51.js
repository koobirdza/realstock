...
export function renderInfoBanner() {
  if (!dom.infoBanner) return;
  const meta = MODE_META[state.mode] || {};
  const helper = meta.helper || 'Count/Issue/Receive/Order = append log ก่อน • view/suggest อัปเดตหลัง nightly 22:00';
  dom.infoBanner.innerHTML = `<strong>${escapeHtml(helper)}</strong>`;
}
...
export function setSaveLocked(locked, label = "") { const saveBtn = $("saveBtn"); if (!saveBtn) return; saveBtn.disabled = !!locked; saveBtn.dataset.originalLabel ||= saveBtn.textContent; saveBtn.textContent = locked ? (label || "Saving...") : saveBtn.dataset.originalLabel; $$('[data-step], [data-suggest], [data-qty-index], #clearBtn, #restoreBtn', dom.itemPanel).forEach((el) => { el.disabled = !!locked; }); }
function itemMetaHtml(mode, item, stock, order) {
  ...
  if (mode === 'issue') return `<div class="meta">ตัดจาก: ${escapeHtml(item.issue_source || item.stock_type || item.target_category || 'ห้องสต๊อก')}</div><div class="meta">ใช้ได้ที่: ${escapeHtml(item.usage_zones || '-')}</div>`;
  if (mode === 'receive') return `<div class="meta">รับเข้า: ${escapeHtml(item.receive_target || item.stock_type || item.target_category || '-')}</div><div class="meta">flow: ${escapeHtml(item.flow_type || 'central')}</div>`;
  return `<div class="meta">snapshot: ${escapeHtml(order.nightly_snapshot_date || stock.nightly_snapshot_date || '-')}</div><div class="meta">threshold: ${escapeHtml(order.threshold_stock ?? '-')} • target: ${escapeHtml(order.target_par ?? '-')} • suggest: ${escapeHtml(order.suggested_order_qty || 0)} ${escapeHtml(item.unit || '')}</div><div class="meta">รอบวันที่: ${escapeHtml(order.schedule_group || item.__scheduleBadge || '-')}</div>`;
}
export function renderItems(node, stockMap = {}, orderRows = [], onSave) {
  const items = getItems(node); dom.nodePanel.classList.add("hidden"); dom.itemPanel.classList.remove("hidden"); const meta = MODE_META[state.mode]; if (!items.length) { dom.itemPanel.innerHTML = '<div class="card pad">ไม่พบรายการในหมวดนี้</div>'; return; }
  const orderMap = Object.fromEntries(orderRows.map((x) => [x.item_key, x]));
  dom.itemPanel.innerHTML = `<div class="card pad"><div class="between" style="margin-bottom:12px"><div><strong>${escapeHtml(meta.label)}</strong><div class="hint">${escapeHtml(meta.helper)}</div></div><div class="pill">${items.length} รายการ</div></div><div class="grid">${items.map((item, idx) => { const stock = stockMap[item.item_key] || {}; const order = orderMap[item.item_key] || {}; const suggestion = Number(order.suggested_order_qty || 0); return `<div class="item"><div class="between" style="margin-bottom:6px;align-items:flex-start"><h4>${idx + 1}. ${escapeHtml(item.item_name)}</h4><span class="mini-badge">${escapeHtml(item.unit || '')}</span></div>${itemMetaHtml(state.mode, item, stock, order)}<div class="qty-row"><input class="input qty" data-qty-index="${idx}" inputmode="decimal" type="number" min="0" step="any" placeholder="จำนวน" />${state.mode === 'order' ? `<button class="btn" data-suggest="${idx}:${suggestion || 0}">ใช้ suggest</button>` : `<button class="btn step" data-step="${idx}:1">+1</button>`}</div></div>`; }).join('')}</div><div class="footer-bar"><div class="row"><button id="saveBtn" class="btn primary">${escapeHtml(meta.saveLabel)}</button><button id="clearBtn" class="btn">ล้างค่า</button><button id="restoreBtn" class="btn">กู้ข้อมูลค้าง</button></div></div></div>`;
  ...
  $$('[data-suggest]', dom.itemPanel).forEach((el) => el.addEventListener('click', () => { const [idx, qty] = el.dataset.suggest.split(':'); const input = dom.itemPanel.querySelector(`[data-qty-index="${idx}"]`); input.value = qty; input.dispatchEvent(new Event('input')); }));
