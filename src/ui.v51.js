...
export function bindDom() {
  ["loginPage","appPage","employeeName","loginBtn","logoutBtn","employeeDisplay","modeBadge","healthBadge","versionLabel","homeBtn","backBtn","breadcrumb","nodePanel","itemPanel","destinationPanel","destinationButtons","toast","errorBox","cacheStamp","adminPanel","warmBtn","nightlyBtn","preflightBtn","diagText","infoBanner","installBtn"].forEach((id) => dom[id] = $(id));
...
export function renderAdmin() {
  ...
  if (state.admin && dom.diagText) dom.diagText.innerHTML = escapeHtml(state.infoBanner || "");
}

export function renderInfoBanner() {
  if (!dom.infoBanner) return;
  const meta = MODE_META[state.mode] || {};
  const helper = meta.helper || "Count/Issue/Receive = append log only • Order = nightly report";
...
export function renderBreadcrumb(tree) {
  ...
  dom.breadcrumb.innerHTML = crumbs.map((c, idx) => `${idx ? '<span class="crumb-sep">›</span>' : ''}<span class="crumb"><span class="crumb-icon">${c.icon || '📁'}</span><span>${escapeHtml(c.label)}</span></span>`).join("");
  dom.cacheStamp.innerHTML = escapeHtml(state.lastCacheStamp || "snapshot ล่าสุดจากรอบ 22:00");
}
...
export function renderDestinationPicker() {
  ...
  if (!visible) return;
  dom.destinationButtons.innerHTML = ISSUE_DESTINATIONS.map((d) => `<button class="btn" data-dest="${escapeHtml(d.key)}" ${state.destination === d.key ? 'data-selected="1"' : ''}>${escapeHtml(d.label)}</button>`).join("");
}
...
export function renderNodes(node, onOpen) {
  const children = Object.values(node?.children || {}).sort((a, b) => String(a.label || "").localeCompare(String(b.label || ""), "th"));
  dom.itemPanel.classList.add("hidden");
  dom.nodePanel.classList.remove("hidden");
  dom.nodePanel.innerHTML = children.length ? children.map((child) => `
    <button class="nav-btn nav-card ${child.type || 'folder'}" data-node="${escapeHtml(child.key || '')}">
      <div class="nav-top">
        <span class="nav-icon">${child.icon || '📁'}</span>
        ${child.badge ? `<span class="mini-badge">${escapeHtml(child.badge)}</span>` : ''}
      </div>
      <div class="nav-title">${escapeHtml(child.label || 'ไม่มีชื่อ')}</div>
      <div class="hint">เปิดหมวด</div>
    </button>`).join("") : '<div class="card pad">ไม่พบหมวด</div>';
  $$('[data-node]', dom.nodePanel).forEach((el) => el.addEventListener('click', () => onOpen(el.dataset.node)));
}
...
export function setSaveLocked(locked, label = "") {
  ...
  $$('[data-step], [data-qty-index], #clearBtn, #restoreBtn', dom.itemPanel).forEach((el) => {
    el.disabled = !!locked;
  });
}
...
function itemMetaHtml(mode, item, stock, order) {
  ...
  return `<div class="meta">snapshot: ${escapeHtml(order.nightly_snapshot_date || '-')} • รอบ: ${escapeHtml(order.schedule_group || item.__scheduleBadge || '-')}</div><div class="meta">threshold: ${escapeHtml(order.threshold_stock ?? '-')} • target: ${escapeHtml(order.target_par ?? '-')}</div>`;
}

export function renderItems(node, stockMap = {}, orderRows = [], onSave) {
  let items = getItems(node);
  ...
  if (!items.length) {
    dom.itemPanel.innerHTML = `<div class="card pad">${state.mode === 'order' ? 'รอบนี้ไม่มีรายการที่ต้องสั่ง' : 'ไม่พบรายการในหมวดนี้'}</div>`;
    return;
  }
