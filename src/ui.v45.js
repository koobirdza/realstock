import { APP_VERSION, ISSUE_DESTINATIONS } from "./config.v45.js";
import { state } from "./state.v45.js";
import { getChildren, getItems, getPathLabels, needsDestination, getModeMeta } from "./catalog.v45.js";
import { escapeHtml, qs } from "./utils.v45.js";
...
export function renderNavigation(node, handlers, stockSummary = {}, orderRows = []) {
  ...
  if (state.mode === "order") {
    const keys = new Set(items.map((x) => x.item_key));
    renderOrder(orderRows.filter((row) => keys.has(row.item_key)));
    return;
  }
