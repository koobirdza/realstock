import { MAX_QTY } from "./config.v45.js";
import { state } from "./state.v45.js";
import { getItems, getNodeByPath, needsDestination } from "./catalog.v45.js";
import { nowIso, todayIso } from "./utils.v45.js";

export function collectRecords(catalogTree, inputRows) {
  if (!state.employee) throw new Error("ยังไม่ได้เข้าสู่ระบบ");
  if (!state.mode) throw new Error("ยังไม่ได้เลือกโหมด");
  if (state.mode === "order") throw new Error("โหมดสั่งของเป็นโหมดดูข้อมูล ไม่ต้องบันทึก");
  ...
  const chosen = inputRows
    ...
    .map((row) => {
      ...
      if (Number.isNaN(qty) || qty < 0 || qty > MAX_QTY) {
        throw new Error(`จำนวนไม่ถูกต้อง: ${item.item_name}`);
      }
  ...
  if (!chosen.length) {
    throw new Error(
      state.mode === "count"
        ? "กรุณากรอกยอดคงเหลืออย่างน้อย 1 รายการ"
        : state.mode === "issue"
          ? "กรุณากรอกจำนวนที่เบิกอย่างน้อย 1 รายการ"
          : "กรุณากรอกจำนวนที่รับเข้าอย่างน้อย 1 รายการ"
    );
  }
  return chosen;
}
