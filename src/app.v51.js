...
function attachInputAutosave() { if (state.mode === 'order') return; $$('[data-qty-index]', document).forEach((el) => el.addEventListener('input', autosaveDraft)); $('clearBtn')?.addEventListener('click', () => { $$('[data-qty-index]', document).forEach((el) => el.value = ''); clearDraft(); toast('ล้างค่าแล้ว'); }); $('restoreBtn')?.addEventListener('click', () => { const draft = getDraft()?.value; if (!draft || draft.mode !== state.mode) return toast('ไม่พบข้อมูลค้าง', 'info'); draft.values.forEach((row) => { const input = document.querySelector(`[data-qty-index="${row.index}"]`); if (input) input.value = row.value; }); toast('กู้ข้อมูลค้างแล้ว', 'success'); }); }
...
function bindEvents() { $('loginBtn').addEventListener('click', async () => { const name = $('employeeName').value.trim(); if (!name) return showError('กรุณากรอกชื่อพนักงาน'); try { setEmployee(name); persistSession(name); showError(''); render(); await ensureBootstrapLoaded(); } catch (err) { console.error(err); showError('เข้าสู่ระบบไม่สำเร็จ'); } }); $('employeeName').addEventListener('keydown', (e) => { if (e.key === 'Enter') $('loginBtn').click(); }); $('logoutBtn').addEventListener('click', () => { logoutSession(); resetNav(); state.mode = ''; state.bootstrapped = false; render(); }); $$('[data-mode]').forEach((el) => el.addEventListener('click', () => chooseMode(el.dataset.mode))); $('homeBtn').addEventListener('click', () => { resetNav(); render(); }); $('backBtn').addEventListener('click', () => { state.path.pop(); render(); }); $('destinationButtons').addEventListener('click', (e) => { const btn = e.target.closest('[data-dest]'); if (!btn) return; state.destination = btn.dataset.dest; renderDestinationPicker(); }); $('warmBtn')?.addEventListener('click', async () => { const res = await adminWarm(); toast(res?.ok ? 'Warm cache สำเร็จ' : 'Warm cache ไม่สำเร็จ', res?.ok ? 'success' : 'error'); refreshDiagnostics(); }); $('nightlyBtn')?.addEventListener('click', async () => { const res = await adminNightly(); toast(res?.ok ? `Nightly สำเร็จ • ${res.orderRows} รายการคำนวณแล้ว` : 'Nightly ไม่สำเร็จ', res?.ok ? 'success' : 'error', 2500); if (res?.ok) { clearDataCaches(); state.bootstrapped = false; await ensureBootstrapLoaded(true); render(); } refreshDiagnostics(); }); $('preflightBtn')?.addEventListener('click', async () => { const pf = await preflight(); toast(pf?.ok ? `Preflight ${pf.summary.status}` : 'Preflight ไม่สำเร็จ', pf?.ok ? 'success' : 'error', 2500); state.infoBanner = pf?.ok ? `preflight: ${pf.summary.status}` : 'preflight failed'; renderAdmin(); }); }
async function bootstrap() { bindDom(); state.admin = params().get('admin') === '1'; restoreSession(); bindEvents(); render(); await refreshHealth(); if (state.employee) { try { await ensureBootstrapLoaded(); } catch (err) {} } await refreshDiagnostics(); if (ENABLE_SERVICE_WORKER && 'serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js').catch(() => {}); }
bootstrap();
