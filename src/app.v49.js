...
function currentWarm() {
  return getWarmMode(state.mode);
}
function currentTree() {
  return currentWarm().tree;
}
function currentNode() {
  return getNodeByPath(currentTree(), state.path) || currentTree();
}
function currentHtml() {
  return currentWarm().html;
}
...
async function ensureCatalogLoaded(mode, force = false) {
  ...
  try {
    await state.loadingModes[mode];
  } finally {
    delete state.loadingModes[mode];
  }
}

async function ensureSideData(mode, force = false) {
  if (mode === 'order') {
  ...
  prewarmPromise = (async () => {
    for (const mode of otherModes) {
      try {
        ...
        console.warn('prewarm mode failed:', mode, err?.message || err);
      }
    }
  })();
  try {
