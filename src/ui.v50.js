import { MODE_META, ISSUE_DESTINATIONS, APP_VERSION } from "./config.v50.js";
import { state } from "./state.v50.js";
import { $, $$, escapeHtml } from "./utils.v50.js";
import { getChildren, getItems, pathLabels, needsDestination, getScheduleBadge } from "./catalog.v50.js";

const dom = {};
export function bindDom() {
  ["loginPage","appPage","employeeName","loginBtn","logoutBtn","employeeDisplay","modeBadge","healthBadge","versionLabel","homeBtn","backBtn","breadcrumb","nodePanel","itemPanel","destinationPanel","destinationButtons","toast","errorBox","cacheStamp","adminPanel","warmBtn","rebuildBtn","syncBadge","runtimePanel","diagPanel","refreshDiagBtn"]
...
export function setSyncBadge(text, type = 'idle') {
  if (!dom.syncBadge) return;
  dom.syncBadge.textContent = text || 'พร้อม';
  dom.syncBadge.dataset.sync = type;
}
export function renderAdmin() {
  dom.adminPanel.classList.toggle("hidden", !state.admin);
}
export function renderRuntime() {
  if (!dom.runtimePanel) return;
  const v = state.runtimeVersions || {};
  dom.runtimePanel.innerHTML = `Frontend: <strong>${escapeHtml(APP_VERSION)}</strong> • Backend: <strong>${escapeHtml(v.backend || '-')}</strong> • Stock v: <strong>${escapeHtml(v.stock || '-')}</strong> • Order v: <strong>${escapeHtml(v.order || '-')}</strong>`;
}
export function renderDiagnostics() {
  if (!dom.diagPanel) return;
  if (!state.admin) { dom.diagPanel.innerHTML = ''; return; }
  const d = state.diagnostics || {};
  dom.diagPanel.innerHTML = `Last save: <strong>${escapeHtml(d.lastSaveAt || '-')}</strong> • Avg elapsed: <strong>${escapeHtml(String(d.avgElapsedMs || '-'))}</strong> ms • Duplicate blocked: <strong>${escapeHtml(String(d.duplicateBlocked || 0))}</strong> • Trace: <strong>${escapeHtml(state.lastTraceId || '-')}</strong>`;
}
    ...
    <div class="card pad">
      ...
      <div class="grid">
        ${items.map((item, idx) => {
          ...
          const pending = state.optimisticPending[item.item_key];
          const pendingHtml = pending ? `<div class="meta">กำลัง sync: ${escapeHtml(String(pending.qty))}</div>` : '';
          return `<div class="item">
            <div class="between" style="margin-bottom:6px;align-items:flex-start">
              <h4>${idx + 1}. ${escapeHtml(item.item_name)}</h4>
              ${badge}
            </div>
            <div class="meta">คงเหลือ ${escapeHtml(stock.current_stock ?? "-")} ${escapeHtml(item.unit || stock.unit || "")}</div>
